  ...
  return (
    <div className="mb-5">
      ...
      <input
        ...
        className="w-full h-1.5 rounded-full appearance-none cursor-pointer accent-[#3B6098]"
        style={{ background: `linear-gradient(to right, #3B6098 0%, #3B6098 ${((value-min)/(max-min))*100}%, ${isDarkMode ? '#1f2937' : '#e2e8f0'} ${((value-min)/(max-min))*100}%, ${isDarkMode ? '#1f2937' : '#e2e8f0'} 100%)` }}
      />
...
function ResultCard({ label, value, highlight, isDarkMode }) {
  return (
    <div className={`rounded-2xl p-4 border text-center ${
      ...
      <p className={`text-xl font-black ${highlight ? 'text-[#5E82BC]' : isDarkMode ? 'text-white' : 'text-slate-900'}`}>{value}</p>
    </div>
  );
}
      ...
      <div className="max-w-2xl mx-auto">
          ...
          {['sip', 'lumpsum'].map(m => (
            <button key={m} onClick={() => setMode(m)}
              className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 ${
                mode === m
                  ? 'bg-[#3B6098] text-white shadow-lg'
        ...
        {mode === 'sip' ? (
          <>
            ...
            <div className={`rounded-2xl border p-6 mb-4 text-center ${
              ...
              <p className={`text-5xl font-black mb-2 ${
                sipRate > 0 && sipRate < 100 ? 'text-[#5E82BC]' : 'text-red-400'
              }`}>
              ...
              {sipRate > 12 && sipRate < 100 && (
                <p className="mt-2 text-xs font-semibold text-[#5E82BC]">🎉 Excellent! Above market average (12%)</p>
              )}
              {sipRate > 0 && sipRate <= 8 && (
          ...
          <>
            <div className={`rounded-2xl border p-6 mb-4 text-center ${
              ...
              <p className={`text-5xl font-black mb-2 ${
                lumpsumRate > 0 && lumpsumRate < 100 ? 'text-[#5E82BC]' : 'text-red-400'
              }`}>
                {lumpsumRate > 0 && lumpsumRate < 100 ? `${lumpsumRate.toFixed(2)}%` : '—'}
              </p>
              <p className={`text-xs ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>per annum</p>
              {lumpsumRate > 12 && lumpsumRate < 100 && (
                <p className="mt-2 text-xs font-semibold text-[#5E82BC]">🎉 Excellent! Above market average</p>
              )}
            </div>
            <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <div className="flex flex-wrap gap-3 justify-center no-print">
          <a href={waLink} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 px-4 py-2.5 bg-[#4C9A82] text-white rounded-xl text-sm font-bold hover:bg-[#5FB89C] transition-colors">
          ...
          <Link to="/sip"
            className="flex items-center gap-2 px-4 py-2.5 bg-[#3B6098]/10 border border-[#3B6098]/30 text-[#5E82BC] rounded-xl text-sm font-bold hover:bg-[#3B6098]/20 transition-colors">
            📈 Plan with SIP →
          </Link>
        </div>
