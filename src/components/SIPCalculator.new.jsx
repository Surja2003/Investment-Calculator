...
const SIPCalculator = () => {
  ...
  const handleAmountChange = (e) => {
    ...
    setFormData(prev => ({
      ...prev,
      monthlyAmount: value ? parseInt(value, 10) : 0,
  ...
  const handleCalculateClick = useCallback(() => {
    ...
    // Log calculation
    console.log('Calculating SIP returns with:', formData);
  ...
  const metrics = useMemo(() => {
    ...
    // Calculate total invested amount considering annual increments
    let totalInvestedAmount = 0;
    let monthlyInvestment = formData.monthlyAmount;
    
    for (let year = 0; year < formData.years; year++) {
      totalInvestedAmount += monthlyInvestment * 12;
      
      // Apply annual increment if applicable
      if (formData.incrementPercent > 0) {
        monthlyInvestment = monthlyInvestment * (1 + formData.incrementPercent / 100);
      }
    }
    
    const gains = Math.max(0, futureValue - totalInvestedAmount);
    const returnPercentage = totalInvestedAmount > 0 ? ((gains / totalInvestedAmount) * 100) : 0;
                ...
                >
                  <CardContent>
                    ...
                    <Typography variant="h5" color="#E5E7EB" sx={{ fontWeight: 'bold', my: 1 }}>
                      <CountUp 
                        to={metrics.totalInvestedAmount} 
                        formatter={(value) => formatCurrency(value)}
                    ...
                    <Typography variant="caption" sx={{ color: 'rgba(255, 255, 255, 0.5)' }}>
                      Monthly: {formatCurrency(formData.monthlyAmount)}
                    </Typography>
                  </CardContent>
                </Card>
              </SlideIn>
              ...
              <SlideIn direction="top" delay={0.15}>
                ...
                >
                  <CardContent>
                    ...
                    <Typography variant="caption" sx={{ color: 'rgba(255, 255, 255, 0.5)' }}>
                      {formData.years * 12} monthly payments
                    </Typography>
                  </CardContent>
                </Card>
              </SlideIn>
                ...
                >
                  <CardContent>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                      ...
                      <Box>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                          <Typography variant="body2" component="label" htmlFor="sip-annual-return">Expected Annual Return</Typography>
                          <Box sx={{ 
                            bgcolor: 'rgba(30, 41, 59, 0.5)',
                            px: 2, 
                            py: 0.5, 
                      ...
                      {/* Annual SIP Increase Option */}
                      <Box>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                          <Typography variant="body2" component="label" htmlFor="sip-annual-increase">Annual SIP Increase</Typography>
                          <Box sx={{ 
                            bgcolor: 'rgba(30, 41, 59, 0.5)',
                            px: 2, 
                            py: 0.5, 
                            ...
                            <Typography variant="body2" fontWeight="medium">{formData.incrementPercent}%</Typography>
                          </Box>
                        </Box>
                        <Range
                          id="sip-annual-increase"
                          name="incrementPercent"
                          value={formData.incrementPercent}
                          min={0}
                          max={20}
                          onChange={handleRangeChange('incrementPercent')}
                        />
                        <AnimatedProgressBar 
                          value={formData.incrementPercent} 
                          max={20} 
                      ...
                      <Box>
                        ...
                        {formData.includeInflation && (
                          <Box sx={{ mt: 2 }}>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                              <Typography variant="body2" component="label" htmlFor="sip-inflation-rate">Inflation Rate</Typography>
                              <Box sx={{ 
                                bgcolor: 'rgba(30, 41, 59, 0.5)',
                                px: 2, 
                                py: 0.5, 
                        ...
                        >
                          ...
                          <Tooltip 
                            formatter={(value, name) => {
                              return [formatCurrency(value), name === 'Invested' ? 'Total Invested' : 'Current Value'];
                            }}
          ...
          <FadeIn delay={0.3}>
            <Box sx={{ mb: 4 }}>
              ...
              <Typography variant="body2" color="text.secondary">
                Compare how your monthly SIP investment would perform under different market conditions.
              </Typography>
            </Box>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '7fr 5fr' }, gap: 3 }}>
              <Box>
                <ScenarioComparison
                  defaultAmount={formData.monthlyAmount}
