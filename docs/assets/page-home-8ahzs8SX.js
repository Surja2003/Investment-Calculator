import{j as e}from"./vendor-mui-ArKiFWmM.js";import{r as m,L as k}from"./vendor-react-DuuoFh8M.js";const C=m.createContext();function X({children:t}){const[s,n]=m.useState(()=>{const a=localStorage.getItem("darkMode");return a?JSON.parse(a):!1});m.useEffect(()=>{localStorage.setItem("darkMode",JSON.stringify(s)),s?document.documentElement.classList.add("dark"):document.documentElement.classList.remove("dark")},[s]);const r=()=>{n(a=>!a)};return e.jsx(C.Provider,{value:{isDarkMode:s,toggleDarkMode:r},children:t})}function w(){return m.useContext(C)}const q="/api";async function y(t,s={}){try{return await(await fetch(`${q}${t}`,{headers:{"Content-Type":"application/json"},...s})).json()}catch(n){return{ok:!1,error:n.message}}}const P={subscribe(t){return y("/newsletter",{method:"POST",body:JSON.stringify({email:t})})},contact(t){return y("/contacts",{method:"POST",body:JSON.stringify(t)})},listNewsletter(){return y("/newsletter",{method:"GET"})},listContacts(){return y("/contacts",{method:"GET"})}},b={newsletter:"newsletter_subscribers",contacts:"contact_messages"};function F(t){try{const s=localStorage.getItem(t);return s?JSON.parse(s):[]}catch{return[]}}function T(t,s){try{return localStorage.setItem(t,JSON.stringify(s)),!0}catch{return!1}}async function $(t){const s=new Date().toISOString();if((await P.subscribe(t))?.ok)return{ok:!0};const r=F(b.newsletter);return r.find(a=>a.email?.toLowerCase()===t.toLowerCase())||(r.push({email:t,date:s}),T(b.newsletter,r)),{ok:!0,fallback:!0}}async function U({name:t,email:s,message:n}){const r=new Date().toISOString();if((await P.contact({name:t,email:s,message:n}))?.ok)return{ok:!0};const l=F(b.contacts);return l.push({name:t,email:s,message:n,date:r}),T(b.contacts,l),{ok:!0,fallback:!0}}const D=300*1e3,g={};function E(t){return g[t]&&Date.now()-g[t].ts<D}const S={"BTC-USD":"bitcoin","ETH-USD":"ethereum","BNB-USD":"binancecoin","SOL-USD":"solana","DOGE-USD":"dogecoin","USDT-USD":"tether","XRP-USD":"ripple","ADA-USD":"cardano"};async function L(t=[]){const s="crypto_"+t.join(",");if(E(s))return g[s].data;const n=t.map(r=>S[r]).filter(Boolean).join(",");if(!n)return[];try{const r=`https://api.coingecko.com/api/v3/simple/price?ids=${n}&vs_currencies=usd&include_24hr_change=true&include_market_cap=false`,a=await fetch(r,{signal:AbortSignal.timeout(8e3)});if(!a.ok)throw new Error("CoinGecko error "+a.status);const l=await a.json(),c=t.map(o=>{const u=S[o];if(!u||!l[u])return null;const p=l[u];return{symbol:o,shortName:o.replace("-USD",""),price:p.usd??null,changePercent:p.usd_24h_change??null,currency:"USD"}}).filter(Boolean);return g[s]={data:c,ts:Date.now()},c}catch(r){return console.warn("[quoteData] CoinGecko fetch failed:",r.message),g[s]?.data??[]}}const W=[{symbol:"^NSEI",shortName:"NIFTY 50",currency:"INR"},{symbol:"^BSESN",shortName:"SENSEX",currency:"INR"},{symbol:"^GSPC",shortName:"S&P 500",currency:"USD"},{symbol:"^DJI",shortName:"Dow Jones",currency:"USD"}];async function A(t){const n=`https://api.allorigins.win/get?url=${encodeURIComponent(`https://query1.finance.yahoo.com/v8/finance/chart/${t}?interval=1d&range=2d`)}`,r=await fetch(n,{signal:AbortSignal.timeout(8e3)});if(!r.ok)throw new Error("proxy error "+r.status);const a=await r.json(),c=JSON.parse(a.contents)?.chart?.result?.[0]?.meta;if(!c)throw new Error("no meta");return{price:c.regularMarketPrice??null,prevClose:c.chartPreviousClose??c.previousClose??null}}async function M(){const t="indices";if(E(t))return g[t].data;const n=(await Promise.allSettled(W.map(r=>A(r.symbol).then(a=>({symbol:r.symbol,shortName:r.shortName,currency:r.currency,price:a.price,changePercent:a.prevClose&&a.price?(a.price-a.prevClose)/a.prevClose*100:null}))))).map(r=>r.status==="fulfilled"?r.value:null).filter(Boolean);return n.length>0&&(g[t]={data:n,ts:Date.now()}),n.length>0?n:g[t]?.data??[]}const G=new Intl.NumberFormat("en-IN",{maximumFractionDigits:0}),O=new Intl.NumberFormat("en-US",{maximumFractionDigits:2}),R=new Intl.NumberFormat("en-US",{maximumFractionDigits:0});function Y(t,s){return t==null?"—":s==="INR"?"₹"+G.format(t):t>=1e3?"$"+R.format(t):"$"+O.format(t)}function N({name:t,price:s,changePercent:n,currency:r,isDarkMode:a,badge:l}){const c=(n??0)>=0,o=c?"▲":"▼",u=n!=null?`${o} ${Math.abs(n).toFixed(2)}%`:"—";return e.jsxs("div",{className:`flex-shrink-0 min-w-[130px] rounded-2xl p-4 border transition-all duration-300 ${a?"bg-[#0c1222]/80 border-slate-800 hover:border-emerald-500/30":"bg-white border-slate-200 hover:border-emerald-400/50 shadow-sm"}`,children:[l&&e.jsx("span",{className:`inline-block text-[9px] font-bold tracking-widest uppercase px-1.5 py-0.5 rounded mb-2 ${a?"bg-slate-800 text-slate-400":"bg-slate-100 text-slate-500"}`,children:l}),e.jsx("div",{className:`text-[11px] font-semibold mb-1 truncate ${a?"text-slate-400":"text-slate-500"}`,children:t}),e.jsx("div",{className:`text-base font-black tracking-tight mb-1 ${a?"text-white":"text-slate-900"}`,children:Y(s,r)}),e.jsx("div",{className:`text-[11px] font-bold px-2 py-0.5 rounded-full inline-block ${c?a?"bg-emerald-950/60 text-emerald-400":"bg-emerald-50 text-emerald-700":a?"bg-red-950/60 text-red-400":"bg-red-50 text-red-600"}`,children:u})]})}function j({isDarkMode:t}){return e.jsxs("div",{className:`flex-shrink-0 min-w-[130px] rounded-2xl p-4 border animate-pulse ${t?"bg-[#0c1222]/80 border-slate-800":"bg-white border-slate-200"}`,children:[e.jsx("div",{className:`h-2 w-12 rounded mb-3 ${t?"bg-slate-800":"bg-slate-200"}`}),e.jsx("div",{className:`h-3 w-20 rounded mb-2 ${t?"bg-slate-800":"bg-slate-200"}`}),e.jsx("div",{className:`h-5 w-24 rounded mb-2 ${t?"bg-slate-800":"bg-slate-200"}`}),e.jsx("div",{className:`h-3 w-16 rounded-full ${t?"bg-slate-800":"bg-slate-200"}`})]})}const B=["BTC-USD","ETH-USD","BNB-USD","SOL-USD"],H=()=>{const{isDarkMode:t}=w(),[s,n]=m.useState([]),[r,a]=m.useState([]),[l,c]=m.useState(!0),[o,u]=m.useState(null),[p,x]=m.useState(!1),i=m.useCallback(async()=>{try{const[d,f]=await Promise.all([M(),L(B)]);n(d),a(f),u(new Date),x(!1)}catch{x(!0)}finally{c(!1)}},[]);m.useEffect(()=>{i();const d=setInterval(i,300*1e3);return()=>clearInterval(d)},[i]);const h=o?o.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"}):null;return e.jsxs("div",{children:[e.jsxs("div",{className:"flex items-center justify-between mb-4",children:[e.jsxs("div",{className:"flex items-center gap-2",children:[e.jsx("span",{className:"w-1.5 h-4 bg-emerald-500 rounded-full inline-block"}),e.jsx("span",{className:`text-xs font-bold uppercase tracking-wider ${t?"text-slate-400":"text-slate-500"}`,children:"Live Market Data"}),!l&&e.jsxs("span",{className:"flex items-center gap-1",children:[e.jsx("span",{className:"w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse inline-block"}),e.jsx("span",{className:`text-[10px] ${t?"text-slate-500":"text-slate-400"}`,children:"LIVE"})]})]}),h&&e.jsxs("button",{onClick:i,className:`text-[10px] px-2 py-1 rounded-lg border transition-colors ${t?"border-slate-700 text-slate-500 hover:text-emerald-400 hover:border-emerald-600":"border-slate-200 text-slate-400 hover:text-emerald-600 hover:border-emerald-400"}`,children:["↻ Updated ",h]})]}),p&&!l&&e.jsx("p",{className:`text-xs text-center py-3 ${t?"text-slate-500":"text-slate-400"}`,children:"⚠ Could not load live data. Check your connection."}),(l||s.length>0)&&e.jsxs("div",{className:"mb-4",children:[e.jsx("p",{className:`text-[10px] font-bold uppercase tracking-widest mb-2 ${t?"text-slate-600":"text-slate-400"}`,children:"Indices"}),e.jsx("div",{className:"flex gap-3 overflow-x-auto pb-1 snap-x snap-mandatory scrollbar-hide",children:l?[1,2,3,4].map(d=>e.jsx(j,{isDarkMode:t},d)):s.map(d=>e.jsx("div",{className:"snap-start",children:e.jsx(N,{name:d.shortName,price:d.price,changePercent:d.changePercent,currency:d.currency,isDarkMode:t,badge:d.currency==="INR"?"NSE":"NYSE"})},d.symbol))})]}),(l||r.length>0)&&e.jsxs("div",{children:[e.jsx("p",{className:`text-[10px] font-bold uppercase tracking-widest mb-2 ${t?"text-slate-600":"text-slate-400"}`,children:"Crypto"}),e.jsx("div",{className:"flex gap-3 overflow-x-auto pb-1 snap-x snap-mandatory scrollbar-hide",children:l?[1,2,3,4].map(d=>e.jsx(j,{isDarkMode:t},d)):r.map(d=>e.jsx("div",{className:"snap-start",children:e.jsx(N,{name:d.shortName,price:d.price,changePercent:d.changePercent,currency:d.currency,isDarkMode:t,badge:"Crypto"})},d.symbol))})]}),e.jsx("p",{className:`text-[9px] mt-3 text-center ${t?"text-slate-700":"text-slate-300"}`,children:"Prices from CoinGecko & Yahoo Finance · Indicative only · Not investment advice"})]})},_=[{id:"all",label:"All Questions",emoji:"💡"},{id:"why",label:"Why Invest",emoji:"🌱"},{id:"where",label:"Where to Invest",emoji:"📍"},{id:"how",label:"How to Invest",emoji:"🚀"},{id:"compare",label:"SIP vs Others",emoji:"⚖️"},{id:"tools",label:"Our Calculators",emoji:"🧮"}],I=[{category:"why",q:"Why should I start investing?",a:`Investing is the most powerful way to beat inflation and build long-term wealth. Money sitting idle in a savings account typically earns 3–4% p.a. — but inflation runs at 5–6%, meaning your money is silently losing purchasing power every year.
...
Switch between modes using the **🇮🇳/🇺🇸 toggle** in the calculator header. Your inputs and all results update instantly.`}];function z({faq:t,index:s,isOpen:n,onToggle:r,isDarkMode:a}){return e.jsxs("div",{className:`border rounded-2xl overflow-hidden transition-all duration-300 ${n?a?"border-emerald-500/40 bg-emerald-950/10":"border-emerald-400/50 bg-emerald-50/60":a?"border-slate-800 bg-[#0c1222]/60 hover:border-slate-700":"border-slate-200 bg-white hover:border-slate-300 shadow-sm"}`,children:[e.jsxs("button",{id:`faq-btn-${s}`,"aria-expanded":n,"aria-controls":`faq-panel-${s}`,onClick:()=>r(s),className:"w-full flex items-start justify-between gap-4 p-5 text-left cursor-pointer",children:[e.jsx("span",{className:`text-sm sm:text-base font-semibold leading-snug transition-colors ${n?"text-emerald-500":a?"text-slate-100":"text-slate-800"}`,children:t.q}),e.jsx("span",{className:`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 ${n?"bg-emerald-500 text-white rotate-45":a?"bg-slate-800 text-slate-400":"bg-slate-100 text-slate-500"}`,children:"+"})]}),e.jsx("div",{id:`faq-panel-${s}`,role:"region","aria-labelledby":`faq-btn-${s}`,className:`transition-all duration-300 ease-in-out overflow-hidden ${n?"max-h-[2000px] opacity-100":"max-h-0 opacity-0"}`,children:e.jsx("div",{className:`px-5 pb-5 pt-0 border-t ${a?"border-slate-800":"border-slate-100"}`,children:e.jsx("div",{className:`mt-4 text-sm leading-relaxed whitespace-pre-line space-y-2 ${a?"text-slate-300":"text-slate-600"}`,children:t.a.split(`
`).map((l,c)=>{if(!l.trim())return e.jsx("br",{},c);const o=l.split(/\*\*(.*?)\*\*/g);return e.jsx("p",{className:"leading-relaxed",children:o.map((u,p)=>p%2===1?e.jsx("strong",{className:a?"text-emerald-400 font-semibold":"text-emerald-700 font-semibold",children:u},p):u)},c)})})})})]})}const Q=()=>{const{isDarkMode:t}=w(),[s,n]=m.useState(null),[r,a]=m.useState("all"),l=o=>{n(u=>u===o?null:o)},c=r==="all"?I:I.filter(o=>o.category===r);return e.jsx("section",{"aria-label":"Frequently Asked Questions",className:`py-16 px-4 transition-colors duration-200 ${t?"bg-[#090d16]":"bg-slate-50"}`,children:e.jsxs("div",{className:"max-w-4xl mx-auto",children:[e.jsxs("div",{className:"text-center mb-10",children:[e.jsxs("span",{className:`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest mb-4 border ${t?"bg-emerald-950/40 border-emerald-800 text-emerald-400":"bg-emerald-50 border-emerald-200 text-emerald-700"}`,children:[e.jsx("span",{className:"w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block"}),"Investor FAQ"]}),e.jsx("h2",{className:`text-3xl sm:text-4xl font-black tracking-tight mb-3 ${t?"text-white":"text-slate-900"}`,children:"Everything you need to know"}),e.jsx("p",{className:`text-base max-w-2xl mx-auto ${t?"text-slate-400":"text-slate-500"}`,children:"From your first investment to retirement planning — all your questions answered clearly."})]}),e.jsx("div",{className:"flex flex-wrap gap-2 justify-center mb-8",children:_.map(o=>e.jsxs("button",{onClick:()=>{a(o.id),n(null)},className:`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all duration-200 ${r===o.id?"bg-emerald-500 text-white border-emerald-500 shadow-lg shadow-emerald-500/20":t?"bg-slate-800/60 border-slate-700 text-slate-400 hover:border-slate-600 hover:text-slate-200":"bg-white border-slate-200 text-slate-500 hover:border-slate-300 hover:text-slate-700"}`,children:[e.jsx("span",{children:o.emoji}),e.jsx("span",{children:o.label})]},o.id))}),e.jsxs("p",{className:`text-center text-xs mb-6 ${t?"text-slate-600":"text-slate-400"}`,children:["Showing ",c.length," question",c.length!==1?"s":""]}),e.jsx("div",{className:"flex flex-col gap-3",children:c.map((o,u)=>e.jsx(z,{faq:o,index:u,isOpen:s===u,onToggle:l,isDarkMode:t},`${r}-${u}`))}),e.jsxs("div",{className:`mt-12 rounded-2xl p-6 text-center border ${t?"bg-emerald-950/20 border-emerald-800/40":"bg-emerald-50 border-emerald-200"}`,children:[e.jsx("p",{className:`text-base font-semibold mb-1 ${t?"text-emerald-300":"text-emerald-800"}`,children:"Ready to start your investment journey?"}),e.jsx("p",{className:`text-sm mb-4 ${t?"text-slate-400":"text-slate-500"}`,children:"Use our free calculators — no sign-up required."}),e.jsx("div",{className:"flex flex-wrap gap-3 justify-center",children:[{to:"/sip",label:"📈 SIP Calculator"},{to:"/lumpsum",label:"💰 Lumpsum Calculator"},{to:"/swp",label:"🏦 SWP Calculator"},{to:"/goals",label:"🎯 Goal Calculator"}].map(o=>e.jsx(k,{to:o.to,className:`px-4 py-2 rounded-xl text-sm font-semibold border transition-all duration-200 ${t?"bg-slate-800 border-slate-700 text-slate-200 hover:bg-emerald-600 hover:border-emerald-500 hover:text-white":"bg-white border-slate-200 text-slate-700 hover:bg-emerald-500 hover:border-emerald-500 hover:text-white shadow-sm"}`,children:o.label},o.to))})]})]})})},V=()=>{const[t,s]=m.useState(""),[n,r]=m.useState(null),[a,l]=m.useState({name:"",email:"",message:""}),[c,o]=m.useState(null),u=async i=>{if(i.preventDefault(),!t)return;r("Saving...");const h=await $(t);r(h?.ok?"Subscribed!":"Failed. Try again"),h?.ok&&s("")},p=async i=>{i.preventDefault();const{name:h,email:d,message:f}=a;if(!h||!d||!f)return;o("Saving...");const v=await U(a);o(v?.ok?"Sent!":"Failed. Try again"),v?.ok&&l({name:"",email:"",message:""})},{isDarkMode:x}=w();return e.jsxs("div",{className:"min-h-screen",children:[e.jsxs("div",{className:x?"relative overflow-hidden bg-[#090d16] min-h-[88dvh] md:min-h-[78vh] flex items-center":"relative overflow-hidden bg-gradient-to-br from-slate-900 via-emerald-950 to-slate-900 min-h-[88dvh] md:min-h-[78vh] flex items-center",children:[e.jsxs("div",{className:"absolute inset-0 pointer-events-none",children:[e.jsx("div",{className:"absolute top-[-10%] left-[10%] w-[500px] h-[500px] bg-emerald-500/10 rounded-full blur-[120px]"}),e.jsx("div",{className:"absolute bottom-[-5%] right-[5%] w-[400px] h-[400px] bg-cyan-500/8 rounded-full blur-[100px]"}),e.jsx("div",{className:"absolute top-[30%] right-[20%] w-[200px] h-[200px] bg-emerald-400/5 rounded-full blur-[60px]"})]}),e.jsx("div",{className:"relative z-10 container mx-auto px-4 py-10 md:py-16",children:e.jsxs("div",{className:"max-w-5xl mx-auto text-center",children:[e.jsxs("span",{className:"inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-1.5 text-xs sm:text-sm font-semibold text-emerald-300 mb-6 backdrop-blur-sm",children:[e.jsx("span",{className:"w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse inline-block"}),"Free · No sign-up · SIP · Lumpsum · SWP · EMI · Goals"]}),e.jsxs("h1",{className:"text-4xl sm:text-5xl md:text-6xl font-black text-white mb-5 leading-tight tracking-tight",children:["Smart"," ",e.jsx("span",{className:"text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400",children:"Investment"})," ","Calculator"]}),e.jsx("p",{className:"text-base sm:text-lg md:text-xl text-slate-300 mb-8 max-w-2xl mx-auto leading-relaxed",children:"Plan SIP, Lumpsum, SWP, EMI & Goals with real-time projections, inflation adjustments and tax estimates. Built for India, works globally."}),e.jsx("div",{className:"flex flex-wrap justify-center gap-3 mb-10",children:[{to:"/sip",label:"📈 SIP Calculator",primary:!0},{to:"/lumpsum",label:"💰 Lumpsum",primary:!1},{to:"/emi",label:"🏠 EMI Calculator",primary:!1},{to:"/goals",label:"🎯 Goal Planner",primary:!1},{to:"/compare",label:"⚖️ Compare",primary:!1}].map(i=>e.jsx(k,{to:i.to,className:i.primary?"px-5 py-2.5 bg-emerald-500 text-white rounded-xl font-bold hover:bg-emerald-400 transition-all duration-200 shadow-lg shadow-emerald-500/25 text-sm":"px-5 py-2.5 bg-white/10 border border-white/20 text-white rounded-xl font-semibold hover:bg-white/20 transition-all duration-200 backdrop-blur-sm text-sm",children:i.label},i.to))}),e.jsx("div",{className:"grid grid-cols-2 sm:grid-cols-4 gap-3 max-w-3xl mx-auto",children:[{icon:"📊",label:"Year-by-Year Charts",desc:"Visual projections"},{icon:"🧾",label:"Tax Estimation",desc:"LTCG & STCG"},{icon:"🌍",label:"India & Global",desc:"₹ & $ modes"},{icon:"📤",label:"Share Results",desc:"WhatsApp & Link"}].map(i=>e.jsxs("div",{className:"rounded-2xl border border-white/10 bg-white/5 p-3 text-center backdrop-blur-sm",children:[e.jsx("div",{className:"text-xl mb-1",children:i.icon}),e.jsx("div",{className:"text-xs font-bold text-white mb-0.5",children:i.label}),e.jsx("div",{className:"text-[10px] text-slate-400",children:i.desc})]},i.label))})]})})]}),e.jsx("div",{className:x?"bg-[#0c1222] border-b border-slate-800":"bg-white border-b border-slate-200",children:e.jsx("div",{className:"container mx-auto px-4 py-6",children:e.jsx("div",{className:x?"max-w-6xl mx-auto rounded-2xl border border-slate-800 bg-[#090d16]/70 p-5":"max-w-6xl mx-auto rounded-2xl border border-slate-200 bg-slate-50 p-5",children:e.jsx(H,{})})})}),e.jsx(Q,{}),e.jsx("div",{className:x?"bg-slate-950/40":"bg-slate-50",children:e.jsxs("div",{className:"container mx-auto px-4 py-12",children:[e.jsxs("div",{className:"max-w-6xl mx-auto mb-8 text-center",children:[e.jsx("h2",{className:x?"text-2xl md:text-3xl font-bold text-gray-100 mb-2":"text-2xl md:text-3xl font-bold text-gray-900 mb-2",children:"Stay connected"}),e.jsx("p",{className:x?"text-sm md:text-base text-gray-400":"text-sm md:text-base text-gray-600",children:"Get market insights and ask planning questions anytime."})]}),e.jsxs("div",{className:"max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 items-start",children:[e.jsxs("div",{className:"form-box",children:[e.jsx("h3",{children:"Subscribe to Our Newsletter"}),e.jsxs("form",{onSubmit:u,children:[e.jsx("label",{className:"sr-only",htmlFor:"newsletter-email",children:"Enter your email"}),e.jsx("input",{id:"newsletter-email",type:"email",className:"input",placeholder:"Enter your email",value:t,onChange:i=>s(i.target.value),required:!0}),e.jsx("button",{type:"submit",className:"button-primary",children:"Subscribe"})]}),n&&e.jsx("p",{className:"mt-2 text-xs",style:{color:"var(--color-text-secondary)"},children:n})]}),e.jsxs("div",{className:"form-box",children:[e.jsx("h3",{children:"Contact Us"}),e.jsxs("form",{onSubmit:p,children:[e.jsx("label",{className:"sr-only",htmlFor:"contact-name",children:"Your name"}),e.jsx("input",{id:"contact-name",type:"text",className:"input",placeholder:"Your name",value:a.name,onChange:i=>l(h=>({...h,name:i.target.value})),required:!0}),e.jsx("label",{className:"sr-only",htmlFor:"contact-email",children:"Your email"}),e.jsx("input",{id:"contact-email",type:"email",className:"input",placeholder:"Your email",value:a.email,onChange:i=>l(h=>({...h,email:i.target.value})),required:!0}),e.jsx("label",{className:"sr-only",htmlFor:"contact-message",children:"Message"}),e.jsx("textarea",{id:"contact-message",className:"input",placeholder:"Message",rows:"4",value:a.message,onChange:i=>l(h=>({...h,message:i.target.value})),required:!0}),e.jsx("button",{className:"button-primary",children:"Send"})]}),c&&e.jsx("p",{className:"mt-2 text-xs",style:{color:"var(--color-text-secondary)"},children:c})]})]})]})})]})},Z=Object.freeze(Object.defineProperty({__proto__:null,default:V},Symbol.toStringTag,{value:"Module"}));export{Z as H,X as T,w as u};
